...
import { EntityExtractor } from '../src/nlp/entity-extractor';
import { RequirementParser, RequirementParserResult } from '../src/nlp/requirement-parser';
...
describe('NLP Integration Tests - Round 2', () => {
  ...
  test('Test 1: 기본 API 요청 → 전체 NLP 파이프라인', async () => {
    ...
    expect(intentResult.confidence).toBeGreaterThanOrEqual(0.5);
    ...
    expect(requirements.confidence).toBeGreaterThanOrEqual(0.5);

    // 통합 검증
    expect(intentResult.intent).toBe('create_api');
    // database와 auth_type은 TextExtractor에서 자동 추론
    // 이 경우 "사용자 관리"에서 User Management가 인식되어 자동 추론됨
    const enabledRequirements = Object.values(requirements)
      .filter(v => typeof v === 'object' && v !== null && (v as any)?.enabled)
      .length;
    expect(enabledRequirements).toBeGreaterThanOrEqual(0);

    console.log('✅ Test 1: 기본 API 요청 통과');
    console.log('   Intent:', intentResult.intent);
    console.log('   Entity count:', Object.keys(entities).filter(k => entities[k as keyof typeof entities].length > 0).length);
    console.log('   Requirements enabled:',
      Object.values(requirements).filter(v => typeof v === 'object' && v?.enabled).length);
  });
  ...
  test('Test 7: 결과 일관성', async () => {
    ...
    // 3번 반복 실행
    const results: RequirementParserResult[] = [];
    for (let i = 0; i < 3; i++) {
      const entities = extractor.extract(prompt);
      const requirements = parser.parse(prompt, entities);
    ...
    console.log(`   활성 요구사항: ${Object.values(firstResult).filter(v => typeof v === 'object' && v !== null && (v as any)?.enabled).length}개`);
  });
